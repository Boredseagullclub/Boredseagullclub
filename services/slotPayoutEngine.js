...
async function processSlotMachineWinners() {
  ...
  for (let win of pendingWins) {
    ...
    const { _id, sourceChain, amount } = win;
    ...
    try {
      if (sourceChain === 'XRPL') {
        ...
        const txResponse = await tokenContract.transfer(cleanRecipient, ethers.parseUnits(amount.toString(), 18), txParams);
      ...
      console.log(`✅ [DISPATCHED] ${amount} SGC to ${destinationAddress}`);
    } catch (innerErr) {
