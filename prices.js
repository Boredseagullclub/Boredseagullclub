...
const xdcProvider = new ethers.JsonRpcProvider(RPC_POOL.XDC, { chainId: 50, name: 'xdc' }, { staticNetwork: true });
const flrProvider = new ethers.JsonRpcProvider(RPC_POOL.FLARE, { chainId: 14, name: 'flare' }, { staticNetwork: true });
const hbarProvider = new ethers.JsonRpcProvider(RPC_POOL.HEDERA, { chainId: 295, name: 'hedera' }, { staticNetwork: true });
...
async function fetchCurrentLedgerPrices() {
  ...
  try {
    ...
    try {
      ...
      const amount2 = sghAmmInfo.result.amm.amount2; // FIXED: Changed from sgcAmmInfo to sghAmmInfo
      const xrpDrops = typeof amount === 'string' ? parseFloat(amount) : parseFloat(amount2);
      const tokenValue = typeof amount === 'object' ? parseFloat(amount.value) : parseFloat(amount2.value);
...
router.get('/live-matrix', async (req, res) => {
  ...
  if (req.query.historical === 'true' && db) {
      ...
      const now = new Date(); 
      let queryFilter = {};
      let limitValue = 2000; // Safe default chart size
      ...
      if (timeframe === '1H') {
        ...
        limitValue = 100000; // Open the floodgates to show all historical records
      }

      // 🦅 NATIVE SEQUENTIAL AGGREGATION: Pulls entries chronologically regardless of minute stamps
      const historyDocs = await db.collection('price_history').aggregate([
        { $match: queryFilter },
        { $sort: { timestamp: -1 } }, // Grabs the newest trailing records backward
        { $limit: limitValue }        // Prevents browser overload while displaying all your points
      ]).toArray();
